...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("prowl security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("prowl configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("prowl config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("prowl doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "prowl config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("prowl config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
