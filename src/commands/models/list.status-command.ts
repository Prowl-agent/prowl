...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("prowl models auth setup-token")}\` or \`${formatCliCommand("prowl configure")}\`.`
          : `Run \`${formatCliCommand("prowl configure")}\` or set an API key env var.`;
