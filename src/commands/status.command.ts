...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Prowl status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("prowl security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("prowl security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.prowl.ai/faq");
  runtime.log("Troubleshooting: https://docs.prowl.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("prowl status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("prowl logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("prowl status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("prowl gateway probe")}`);
  }
}
