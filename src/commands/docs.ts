...
const SEARCH_TOOL = "https://docs.prowl.ai/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.prowl.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('prowl docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.prowl.ai/");
      runtime.log(`Search: ${formatCliCommand('prowl docs "your query"')}`);
    }
    return;
  }
