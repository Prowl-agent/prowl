...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Prowl Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(prowl completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(prowl completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "prowl") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c prowl -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'prowl gateway' and not 'prowl other gateway', we need to check the sequence.
