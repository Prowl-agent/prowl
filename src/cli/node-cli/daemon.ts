...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("prowl node install"), formatCliCommand("prowl node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("prowl node install --force")}`);
    }
    return;
  }
