...
${formatHelpExamples([
  ['prowl agent --to +15555550123 --message "status update"', "Start a new session."],
  ['prowl agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'prowl agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'prowl agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['prowl agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'prowl agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.prowl.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.prowl.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['prowl agents set-identity --agent main --name "Prowl" --emoji "🦞"', "Set name + emoji."],
  ["prowl agents set-identity --agent main --avatar avatars/prowl.png", "Set avatar path."],
  [
    "prowl agents set-identity --workspace ~/.prowl/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "prowl agents set-identity --identity-file ~/.prowl/workspace/IDENTITY.md --agent main",
