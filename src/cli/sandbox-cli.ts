...
const SANDBOX_EXAMPLES = {
  main: [
    ["prowl sandbox list", "List all sandbox containers."],
    ["prowl sandbox list --browser", "List only browser containers."],
    ["prowl sandbox recreate --all", "Recreate all containers."],
    ["prowl sandbox recreate --session main", "Recreate a specific session."],
    ["prowl sandbox recreate --agent mybot", "Recreate agent containers."],
    ["prowl sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["prowl sandbox list", "List all sandbox containers."],
    ["prowl sandbox list --browser", "List only browser containers."],
    ["prowl sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["prowl sandbox recreate --all", "Recreate all containers."],
    ["prowl sandbox recreate --session main", "Recreate a specific session."],
    ["prowl sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["prowl sandbox recreate --browser --all", "Recreate only browser containers."],
    ["prowl sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["prowl sandbox explain", "Show effective sandbox config."],
    ["prowl sandbox explain --session agent:main:main", "Explain a specific session."],
    ["prowl sandbox explain --agent work", "Explain an agent sandbox."],
    ["prowl sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.prowl.ai/cli/sandbox")}\n`,
    )
    .action(() => {
