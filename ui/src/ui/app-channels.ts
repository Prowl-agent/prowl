...
export async function handleWhatsAppStart(host: ProwlApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ProwlApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ProwlApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ProwlApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ProwlApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ProwlApp): string {
...
function resolveGatewayHttpAuthHeader(host: ProwlApp): string | null {
...
function buildGatewayHttpHeaders(host: ProwlApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ProwlApp) {
...
export function handleNostrProfileToggleAdvanced(host: ProwlApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ProwlApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ProwlApp) {
  const state = host.nostrProfileFormState;
