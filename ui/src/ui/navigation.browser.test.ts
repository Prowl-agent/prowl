...
const originalConnect = ProwlApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("prowl-app") as ProwlApp;
...
beforeEach(() => {
  ProwlApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__PROWL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ProwlApp.prototype.connect = originalConnect;
  window.__PROWL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/prowl/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/prowl");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/prowl/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PROWL_CONTROL_UI_BASE_PATH__ = "/prowl";
    const app = mountApp("/prowl/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/prowl");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/prowl/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("prowl.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
